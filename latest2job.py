...
# -------- Locate testbed file --------
def find_testbed_file(filename="testbedits01.yaml", search_path="."):
    for root, dirs, files in os.walk(search_path):
        if filename in files:
            return os.path.join(root, filename)
    raise FileNotFoundError(f"Testbed file '{filename}' not found.")

testbed_path = find_testbed_file()
print(f"Loading testbed from: {testbed_path}")
testbed = load(testbed_path)

# -------- Ping and parsing logic --------
bgpsectestoutput = []
...
for entry in data:
    ...
    html_content += f"""
        <tr class="{css_class}">
            ...
            <td>
                {status}<br>
                <span class="collapsible" onclick="toggleContent('{content_id}')">[Show/Hide Details]</span>
